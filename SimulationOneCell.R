...
MySimulationCell <- function(Design = Design, RowOfDesign = Row, K = 100){
  #Create matrix or dataframe to store the results:
  MyResult <- matrix(NA, nrow = K*4, ncol = 5)
  ...
  for (k in 1:K) {
    ...
    model <- glm(y ~ t + y_1 + mother.smoke, data = SimDat[[1]], family = binomial, maxit = 50)
    
    #Extract the logistic coefficients
    model_coef <- data.frame(coef(model))
    coef_array <- as.vector(t(model_coef))
    MyResult[r_res, ] <- c(names(SimDat)[1], coef_array)
    r_res <- r_res + 1
    
    #Fit multinomial logistic regression
    for (q in 2:4) {
      model <- vglm(y ~ t + y_1 + mother.smoke, control = vglm.control(maxit = 50), family = multinomial(refLevel = "0"), data = SimDat[[q]])
      #Extract model coefficients
      model_coef <- data.frame(Coef(model))
      model_coef <- model_coef %>% filter(str_ends(row.names(model_coef), "1"))
      model_coef <- model_coef[-which(row.names(model_coef) == "y_1m:1"), ]
      MyResult[r_res, ] <- c(names(SimDat)[q], model_coef)
      r_res <- r_res + 1
    }
  }
  return(MyResult)
