...
var.names <- c("Intercept", "Time point", "Previous response", "Mother smokes")
true.model <- c(-0.293, -0.243, 2.211, 0.296)
...
for (i in 5:8) {
    print(estimates_population %>% group_by(samp, prop) %>% 
...
a <- mvn(estimates_population[, 5:8], mvnTest = "mardia", multivariatePlot = "qq", multivariateOutlierMethod = "quan", showOutliers = T)
...
# MANOVA test
manova.test <- manova(cbind(Intercept, timepoint, Prev_resp_1, mother_smoke) ~ samp*prop*Type, data = estimates_population)
...
#Individual ANOVA
intercept.aov <- estimates_population %>% anova_test(Intercept ~ samp*prop*Type)
intercept.aov
time.aov <- estimates_population %>% anova_test(timepoint ~ samp*prop*Type)
time.aov
prev.aov <- estimates_population %>% anova_test(Prev_resp_1 ~ samp*prop*Type)
prev.aov
mothers.aov <- estimates_population %>% anova_test(mother_smoke ~ samp*prop*Type)
mothers.aov
